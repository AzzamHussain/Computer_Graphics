...
public:
    ...
    void moveSpider() {
        if (!isMoving) return; // Don't move if it's not moving
...
void bullet_movement(int y) {
    ...
    if (bullet.y < screenHeight && isBulletMoving) { // Allow bullet to go fully upwards
        glutTimerFunc(5, bullet_movement, y);
    }
    else if (bullet.y >= screenHeight) { // When bullet reaches the top
        bullet.setPosition(bulletX, 20);
        glutPostRedisplay();
    }
}
